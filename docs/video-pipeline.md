...
- **重复下发会重新下载**：agent 侧没有「文件已存在就跳过」的判断，
  每次下发都会重新走一遍 HTTP 传输并覆盖同名文件。
  同一个视频下发 5 次就是下载 5 次。（这是当前的实现事实，
  不是设计选择——若嫌慢可以加 `stat` 比对后复用。）
- **设备主机的暂存目录不会自动清理**：`removeDownloadedVideo()` 已实现，
  但生产代码里没有任何地方调用它，所以 `<MEDIA_DIR>` 会按 videoId 一直累积。
  需要用 `deploy/relay-data.sh` 或手工清理。
